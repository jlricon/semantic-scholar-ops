...
--tsvector_update_trigger(title_tokens, 'pg_catalog.english', title, paper_abstract, pubyear);
...
--ADD COLUMN title_tokens tsvector GENERATED ALWAYS AS (to_tsvector('english',coalesce(title,''))||to_tsvector('english',coalesce(paper_abstract,''))
--||to_tsvector('english',coalesce(pubyear::varchar,''))) STORED;